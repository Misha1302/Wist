    ...
    where TValue : struct
{
    ...
    public WistGlossary(int capacity)
    {
        ...
        _entries = new WistEntry[capacity];
        _freeCount = 0;
        _freeList = 0;
        _length = 0;
    }
    ...
    public ref TValue Add(int key)
    {
        ...
        while ((uint)i < (uint)entries.Length)
        {
            ...
            if (existsEntry.Key == key) WistGlossaryHelper.KeyAlreadyExists(key);
    ...
    public readonly ref TValue GetValue(int key)
    {
        ...
        WistGlossaryHelper.KeyNotFound(key);
    ...
    public void TrimExcess()
    {
        if (_length == 0)
        {
            ...
            _entries = Array.Empty<WistEntry>();
            _freeCount = 0;
            _freeList = -1;
        ...
        var entries = new WistEntry[newSize];
        var count = 0;
    ...
    {
        var length = _length;
        var newSize = WistGlossaryHelper.GetPrime(length == 0 ? 4 : length << 1);
    ...
    public ref struct WistEnumerator
    {
        private int _index;
        private readonly int _length;
        private readonly WistEntry[] _entries;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public WistEnumerator(WistEntry[] entries, int length)
        {
            _index = -1;
            _length = length;
            _entries = entries;
        ...
        public readonly ref WistEntry Current
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
